...
HEADERS = ['name', 'number', 'email', 'balance']

def to_csv_row
  CSV::Row.new(HEADERS, [name, number, email, balance])
end
end
#end of the class Account

def createnewaccount
  puts "Please enter the following to create a new account: "
  print "Full Name: "
  $xname = gets.chomp
  ...
  puts "WELCOME, #{$xname}! Now you can buy using your FoodHub card! Thank you!"
  ...
  CSV.open('customerdatabase.csv', 'a+') do |csv|
    csv << newperson.to_csv_row
  end
end
...
def existingaccount
  ...
  CSV.foreach('customerdatabase.csv', headers: true) do |row|
    showmetheaccount = Account.from_csv_row(row)
      ...
      puts "\nWelcome back, #{showmetheaccount.name}!\nYou have #{showmetheaccount.balance} pesos. Have fun choosing your food!"
      puts "Also, if you want to top up your account, please don't hesistate to approach the Calle Uno Food Hub admin\nto add more money to your account to ease your food buying experience."
      $xbalance = showmetheaccount.balance
      $xname = showmetheaccount.name
      end
  end
end
...
linedesign
    puts "\nOkay, #{$xname}, did you have fun choosing your food?\nYou have a #{$qtytotal} items in your tray, and your grand total is #{$totaltotal}."
    if $xbalance < $totaltotal
    extra = $totaltotal-$xbalance
    puts "Oops! You are #{extra} over your budget. Please pay the remaining to the nearest admin.\nThanks, and enjoy your meal!"
    $wantmore = 'n'
    else
    puts "This amount will be taken from your account. You now have #{$xbalance-$totaltotal} in your account.
    Enjoy your meal!"
    $wantmore = 'n'
    exit
end
